package acc_txn_pkg;
    ...
    // Inputs
      ...
      rand logic                        pixel_dropped;
      rand logic                        pixel_last;
      rand logic [OUT_MEM_ADDR_W-1 : 0] output_raddr;
      rand logic [7:0]                  pixel_in;

      // CFG PATH
      rand logic                        kernel_we;
      rand logic [K_MEM_ADDR_W-1 : 0]   kernel_waddr;
      rand logic signed [7:0]           kernel_wdata;
    ...
    // CRT
      ...
      constraint pixel_valid_c {
        pixel_valid dist {0 := 1, 1 := 9};
      }
      ...
      constraint pixel_dropped_c {
		...
		`uvm_object_utils_begin (acc_txn)
			...
			// `uvm_field_sarray_int(pixel_in,   	 	UVM_DEFAULT)
			// `uvm_field_sarray_int(pixel_dropped,     UVM_DEFAULT)
			// `uvm_field_sarray_int(idle_cycles,       UVM_DEFAULT)
			// `uvm_field_int		(kernel_we,     	UVM_DEFAULT)
			// `uvm_field_int		(kernel_waddr,      UVM_DEFAULT)
			// `uvm_field_sarray_int(kernel_wdata,      UVM_DEFAULT)
			// `uvm_field_sarray_int(kernel_idle_cycles,UVM_DEFAULT)
      ...
      `uvm_field_int(start,         UVM_DEFAULT)
      `uvm_field_int(pixel_valid,   UVM_DEFAULT)
      `uvm_field_int(pixel_dropped, UVM_DEFAULT)
      `uvm_field_int(pixel_last,    UVM_DEFAULT)
      `uvm_field_int(output_raddr,  UVM_DEFAULT)
      `uvm_field_int(pixel_in,      UVM_DEFAULT)
      `uvm_field_int(kernel_we,     UVM_DEFAULT)
      `uvm_field_int(kernel_waddr,  UVM_DEFAULT)
      `uvm_field_int(kernel_wdata,  UVM_DEFAULT)
